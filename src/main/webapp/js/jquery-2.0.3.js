...
(function(window, undefined) {
	...
	jQuery
			.extend({
				...
				type : function(obj) {
					...
					return typeof obj === "object" || typeof obj === "function" ? class2type[core_toString
							.call(obj)]
							|| "object"
							: typeof obj;
				},
				...
				makeArray : function(arr, results) {
					...
					if (arr != null) {
						if (isArraylike(Object(arr))) {
							jQuery.merge(ret, typeof arr === "string" ? [ arr ]
									: arr);
						} else {
	...
	/*
	 * ! Sizzle CSS Selector Engine v1.9.4-pre http://sizzlejs.com/
	 * 
	...
	(function(window, undefined) {
		...
		rsibling = new RegExp(whitespace + "*[+~]"), rattributeQuotes = new RegExp(
		...
		function siblingCheck(a, b) {
			var cur = b && a, diff = cur && a.nodeType === 1
					&& b.nodeType === 1 && (~b.sourceIndex || MAX_NEGATIVE)
		...
		setDocument = Sizzle.setDocument = function(node) {
			...
			// Tag
			Expr.find["TAG"] = support.getElementsByTagName ? function(tag,
					context) {
			...
			if ((support.matchesSelector = rnative
					.test((matches = docElem.webkitMatchesSelector
			...
			contains = rnative.test(docElem.contains)
				...
				var adown = a.nodeType === 9 ? a.documentElement : a, bup = b
						&& b.parentNode;
				return a === bup
			...
			sortOrder = docElem.compareDocumentPosition ? function(a, b) {
				...
				if (compare) {
					...
					if (compare
						...
						return sortInput ? (indexOf.call(sortInput, a) - indexOf
								.call(sortInput, b))
								: 0;
					}
				...
				return a.compareDocumentPosition ? -1 : 1;
					...
					: function(a, b) {
						...
						if (a === b) {
							...
							// Parentless nodes are either documents or
							// disconnected
						} else if (!aup || !bup) {
							return a === doc ? -1 : b === doc ? 1 : aup ? -1
									: bup ? 1 : sortInput ? (indexOf.call(
											sortInput, a) - indexOf.call(
											sortInput, b)) : 0;

							// If the nodes are siblings, we can do a quick
							// check
						} else if (aup === bup) {
		...
		Sizzle.attr = function(elem, name) {
			...
			val = fn && hasOwn.call(Expr.attrHandle, name.toLowerCase()) ? fn(
					elem, name, !documentIsHTML) : undefined;

			return val === undefined ? support.attributes || !documentIsHTML ? elem
					.getAttribute(name)
		...
		Expr = Sizzle.selectors = {
						...
						return !!elem.parentNode;
							...
							function(elem, context, xml) {
								...
								if (parent) {
									...
									// non-xml :nth-child(...) stores cache data
									// on `parent`
									if (forward && useCache) {
										...
										// Use previously-cached element index
										// if available
									} else if (useCache
										...
										// Use the same loop as above to seek
										// `elem` from the start
										while ((node = ++nodeIndex && node
												&& node[dir]
												|| (diff = nodeIndex = 0)
				...
				"PSEUDO" : function(pseudo, argument) {
					...
					if (fn.length > 1) {
						...
						return Expr.setFilters.hasOwnProperty(pseudo
								.toLowerCase()) ? markFunction(function(seed,
								matches) {
			...
			pseudos : {
				...
				"not" : markFunction(function(selector) {
					...
					return matcher[expando] ? markFunction(function(seed,
							matches, context, xml) {
			...
			return markFunction(function(seed, results, context, xml) {
				...
				if (seed) {
					if (postFinder || preFilter) {
						...
						while (i--) {
							if ((elem = matcherOut[i])
									&& (temp = postFinder ? indexOf.call(seed,
											elem) : preMap[i]) > -1) {
					...
					matcherOut = condense(matcherOut === results ? matcherOut
							.splice(preexisting, matcherOut.length)
							: matcherOut);
					if (postFinder) {
			...
			matchContext = addCombinator(function(elem) {
				...
				return (!leadingRelative && (xml || context !== outermostContext))
						|| ((checkContext = context).nodeType ? matchContext(
								elem, context, xml) : matchAnyContext(elem,
								context, xml));
			} ];
	...
	jQuery
			.extend({
				...
				when : function(subordinate /* , ..., subordinateN */) {
					...
					deferred = remaining === 1 ? subordinate : jQuery
							.Deferred(),
					...
					updateFunc = function(i, contexts, values) {
						return function(value) {
							contexts[i] = this;
							values[i] = arguments.length > 1 ? core_slice
	...
	jQuery.support = (function(support) {
		var input = document.createElement("input"), fragment = document
				.createDocumentFragment(), div = document.createElement("div"), select = document
				.createElement("select"), opt = select.appendChild(document
		...
		jQuery(function() {
			...
			divReset = "padding:0;margin:0;border:0;display:block;-webkit-box-sizing:content-box;-moz-box-sizing:content-box;box-sizing:content-box", body = document
			...
			if (window.getComputedStyle) {
				...
				support.reliableMarginRight = !parseFloat((window
						.getComputedStyle(marginDiv, null) || {}).marginRight);
			}
	 ...
	 * 
	 * 1. Enforce API surface and semantic compatibility with 1.9.x branch 2.
	 * Improve the module's maintainability by reducing the storage paths to a
	 * single mechanism. 3. Use the same single mechanism to support "private"
	 * and "user" data. 4. _Never_ expose "private" data to user code (TODO:
	 * Drop _data, _removeData) 5. Avoid exposing implementation details on user
	 * objects (eg. expando properties) 6. Provide a clear path for
	 * implementation upgrade to WeakMap in 2014
	 */
	...
	Data.accepts = function(owner) {
		...
		// - Object
		// - Any
		return owner.nodeType ? owner.nodeType === 1 || owner.nodeType === 9
				: true;
	};
	...
	Data.prototype = {
		...
		set : function(owner, data, value) {
			...
			if (typeof data === "string") {
				...
				if (jQuery.isEmptyObject(cache)) {
					...
					// Otherwise, copy the properties one-by-one to the cache
					// object
				} else {
					for (prop in data) {
		...
		access : function(owner, key, value) {
			...
			if (key === undefined
				...
				return stored !== undefined ? stored : this.get(owner, jQuery
						.camelCase(key));
			}
	...
	jQuery
			.extend({
					...
					if (elem) {
						...
						if (data) {
							if (!queue || jQuery.isArray(data)) {
								queue = data_priv.access(elem, type, jQuery
										.makeArray(data));
							} else {
								queue.push(data);
							}
				...
				// not intended for public consumption - generates a queueHooks
	...
	jQuery.fn
			.extend({
				...
				addClass : function(value) {
					var classes, elem, cur, clazz, j, i = 0, len = this.length, proceed = typeof value === "string"
							&& value;
				...
				removeClass : function(value) {
					var classes, elem, cur, clazz, j, i = 0, len = this.length, proceed = arguments.length === 0
				...
				toggleClass : function(value, stateVal) {
					...
					if (typeof stateVal === "boolean" && type === "string") {
						return stateVal ? this.addClass(value) : this
								.removeClass(value);
					}
					...
					return this
							.each(function() {
								if (type === "string") {
								...
								} else if (type === core_strundefined
										|| type === "boolean") {
									if (this.className) {
									...
									// If the element has a class name or if
									// we're passed "false",
									// then remove the whole classname (if there
									// was one, the above saved it).
									// Otherwise bring back whatever was
									// previously saved (if anything),
									// falling back to the empty string if
									// nothing was stored.
									this.className = this.className
											|| value === false ? "" : data_priv
			...
			.extend({
					...
					select : {
						get : function(elem) {
							...
							for (; i < max; i++) {
								...
								// IE6-9 doesn't update selected after form
								// reset (#2551)
						...
						set : function(elem, value) {
							...
							while (i--) {
								option = options[i];
								if ((option.selected = jQuery.inArray(jQuery(
										option).val(), values) >= 0)) {
									optionSet = true;
								}
							}
					...
					type : {
						set : function(elem, value) {
							if (!jQuery.support.radioValue && value === "radio"
								...
								// Reset value to default in case type is set
								// after value during creation
								var val = elem.value;
				...
				prop : function(elem, name, value) {
					...
					if (value !== undefined) {
						return hooks
								&& "set" in hooks
								&& (ret = hooks.set(elem, value, name)) !== undefined ? ret
								: (elem[name] = value);

					} else {
						return hooks && "get" in hooks
	...
	jQuery.each(jQuery.expr.match.bool.source.match(/\w+/g), function(i, name) {
		...
		jQuery.expr.attrHandle[name] = function(elem, name, isXML) {
			...
			(jQuery.expr.attrHandle[name] = undefined) != getter(elem, name,
					isXML) ?
	...
	jQuery.each([ "tabIndex", "readOnly", "maxLength", "cellSpacing",
	...
	jQuery.event = {
		...
		trigger : function(event, data, elem, onlyHandlers) {
			...
			var i, cur, tmp, bubbleType, ontype, handle, special, eventPath = [ elem
					|| document ], type = core_hasOwn.call(event, "type") ? event.type
					: event, namespaces = core_hasOwn.call(event, "namespace") ? event.namespace
					.split(".")
					: [];
			...
			if (!onlyHandlers && !event.isDefaultPrevented()) {
				...
				if ((!special._default || special._default.apply(eventPath
						.pop(), data) === false)
		...
		dispatch : function(event) {
			...
			var i, j, ret, matched, handleObj, handlerQueue = [], args = core_slice
					.call(arguments), handlers = (data_priv.get(this, "events") || {})[event.type]
			...
			.extend({
				...
				closest : function(selectors, context) {
					var cur, i = 0, l = this.length, matched = [], pos = (rneedsContext
							.test(selectors) || typeof selectors !== "string") ? jQuery(
					...
					return this.pushStack(matched.length > 1 ? jQuery
							.unique(matched) : matched);
				},
	...
	jQuery.extend({
		filter : function(expr, elems, not) {
			...
			return elems.length === 1 && elem.nodeType === 1 ? jQuery.find
					.matchesSelector(elem, expr) ? [ elem ] : [] : jQuery.find
	...
	jQuery.fn
			.extend({
				...
				remove : function(selector, keepData) {
					var elem, elems = selector ? jQuery.filter(selector, this)
							: this, i = 0;
					...
					return jQuery
							.access(
									...
									function(value) {
										...
										// See if we can take a shortcut and
										// just use innerHTML
										if (typeof value === "string"
											...
											try {
												...
												// If using innerHTML throws an
												// exception, use the fallback
												// method
											} catch (e) {
											}
										}
				...
				replaceWith : function() {
					var
					// Snapshot the DOM in case .domManip sweeps something
					// relevant into its fragment
					...
					this.domManip(arguments, function(elem) {
						...
						if (parent) {
							// Don't use the snapshot next if it has moved
							// (#13810)
						...
						// Allow new content to include elements from the
						// context set
					}, true);

					// Force removal if there was no new content (e.g., from
					// empty arguments)
				...
				domManip : function(args, callback, allowIntersection) {
					...
					if (isFunction
							|| !(l <= 1 || typeof value !== "string"
									|| jQuery.support.checkClone || !rchecked
									.test(value))) {
					...
					function(name, original) {
						jQuery.fn[name] = function(selector) {
							...
							for (; i <= last; i++) {
								...
								// Support: QtWebKit
								// .get() because core_push.apply(_, arraylike)
								// throws
			...
			.extend({
				clone : function(elem, dataAndEvents, deepDataAndEvents) {
					var i, l, srcElements, destElements, clone = elem
							.cloneNode(true), inPage = jQuery.contains(
				...
				buildFragment : function(elems, context, scripts, selection) {
					var elem, tmp, tag, wrap, contains, j, i = 0, l = elems.length, fragment = context
							.createDocumentFragment(), nodes = [];
					...
					for (; i < l; i++) {
						...
						if (elem || elem === 0) {
							...
							if (jQuery.type(elem) === "object") {
								// Support: QtWebKit
								// jQuery.merge because core_push.apply(_,
								// arraylike) throws
	...
	function manipulationTarget(elem, content) {
		return jQuery.nodeName(elem, "table")
				&& jQuery.nodeName(content.nodeType === 1 ? content
	...
	function getAll(context, tag) {
		...
		return tag === undefined || tag && jQuery.nodeName(context, tag) ? jQuery
				.merge([ context ], ret)
				: ret;
	}
	...
	rdisplayswap = /^(none|table(?!-c[ea]).+)/, rmargin = /^margin/, rnumsplit = new RegExp(
			"^(" + core_pnum + ")(.*)$", "i"), rnumnonpx = new RegExp("^("
			+ core_pnum + ")(?!px)[a-z%]+$", "i"), rrelNum = new RegExp(
	...
	cssExpand = [ "Top", "Right", "Bottom", "Left" ], cssPrefixes = [ "Webkit",
			"O", "Moz", "ms" ];
			...
			return jQuery.access(this,
					function(elem, name, value) {
						...
						if (jQuery.isArray(name)) {
							...
							for (; i < len; i++) {
								map[name[i]] = jQuery.css(elem, name[i], false,
										styles);
							}
						...
						return value !== undefined ? jQuery.style(elem, name,
			...
			.extend({
				...
				style : function(elem, name, value, extra) {
					...
					if (value !== undefined) {
						...
						if (!jQuery.support.clearCloneStyle && value === ""
				...
				css : function(elem, name, extra, styles) {
					...
					if (extra === "" || extra) {
						num = parseFloat(val);
						return extra === true || jQuery.isNumeric(num) ? num || 0
								: val;
					}
					return val;
				}
	...
	function getWidthOrHeight(elem, name, extra) {
		...
		var valueIsBorderBox = true, val = name === "width" ? elem.offsetWidth
				: elem.offsetHeight, styles = getStyles(elem), isBorderBox = jQuery.support.boxSizing
		...
		return (val + augmentWidthOrHeight(elem, name, extra
				|| (isBorderBox ? "border" : "content"), valueIsBorderBox,
				styles))
				+ "px";
	}
	...
	function actualDisplay(name, doc) {
		var elem = jQuery(doc.createElement(name)).appendTo(doc.body), display = jQuery
				.css(elem[0], "display");
		elem.remove();
		return display;
	}
					...
					function(i, name) {
						jQuery.cssHooks[name] = {
							get : function(elem, computed, extra) {
								if (computed) {
									// certain elements can have dimension info
									// if we invisibly show them
									// however, it must have a current display
									// style that would benefit from this
									return elem.offsetWidth === 0
											&& rdisplayswap.test(jQuery.css(
													elem, "display")) ? jQuery
				...
				jQuery.cssHooks[prop] = {
					get : function(elem, computed) {
						if (computed) {
							...
							return rnumnonpx.test(computed) ? jQuery(elem)
									.position()[prop]
									+ "px" : computed;
						}
					}
			...
			.extend({
				...
				serializeArray : function() {
					return this
							.map(
									function() {
										// Can add propHook for "elements" to
										// filter or add form elements
										var elements = jQuery.prop(this,
												"elements");
										return elements ? jQuery
												.makeArray(elements) : this;
									})
							.filter(
									function() {
										var type = this.type;
										// Use .is(":disabled") so that
										// fieldset[disabled] works
										return this.name
												&& !jQuery(this)
														.is(":disabled")
												&& rsubmittable
	...
	jQuery.fn.extend({
		...
		undelegate : function(selector, types, fn) {
			...
			return arguments.length === 1 ? this.off(selector, "**") : this
	...
	function addToPrefiltersOrTransports(structure) {
		...
		return function(dataTypeExpression, func) {
			...
			var dataType, i = 0, dataTypes = dataTypeExpression.toLowerCase()
					.match(core_rnotwhite)
					|| [];
			...
			jQuery.ajax({
				...
				data : params
					...
					function(responseText) {
						...
						// elements in a dummy div
						// Exclude scripts to avoid IE 'Permission Denied'
						// errors
	...
	jQuery
			.extend({
				...
				ajaxSettings : {
					...
					// Data converters
					// Keys separate source (or catchall "*") and destination
				...
				ajax : function(url, options) {
					...
					// Context for global events is callbackContext if it is a
					// DOM node or jQuery collection
					...
					jqXHR = {
						...
						statusCode : function(map) {
							...
							if (map) {
								if (state < 2) {
									for (code in map) {
										// Lazy-add the new callback in a way
					...
					// Save the URL in case we're toying with the
					// If-Modified-Since
					...
					if (!s.hasContent) {
						...
						if (s.cache === false) {
							...
							// If there is already a '_' parameter, set its
							// value
							...
							jqXHR
									.always(function() {
										...
										if (s[callbackName]) {
											...
											// options doesn't screw things
											// around
											s.jsonpCallback = originalSettings.jsonpCallback;

											// save the callback name for future
											// use
				...
				if (jQuery.support.cors || xhrSupported && !options.crossDomain) {
					return {
						send : function(headers, complete) {
							...
							// for a preflight are
							// akin to a jigsaw puzzle, we simply never set it
							// to be sure.
							// (it can always be set on a per-request basis or
							// even using ajaxSetup)
									...
									if (callback) {
										...
										if (type === "abort") {
											...
											complete(
													...
													xhr.statusText,
													// Support: IE9
													// #11426: When requesting
													// binary data, IE9 will
													// throw an exception
													// on any attempt to access
													// responseText
	...
	var fxNow, timerId, rfxtypes = /^(?:toggle|show|hide)$/, rfxnum = new RegExp(
			"^(?:([+-])=|)(" + core_pnum + ")([a-z%]*)$", "i"), rrun = /queueHooks$/, animationPrefilters = [ defaultPrefilter ], tweeners = {
		"*" : [ function(prop, value) {
			var tween = this.createTween(prop, value), target = tween.cur(), parts = rfxnum
					.exec(value), unit = parts && parts[3]
			...
			if (parts) {
				...
				// animation
				tween.end = parts[1] ? start + (parts[1] + 1) * parts[2]
						: +parts[2];
			}
	...
	function Animation(elem, properties, options) {
		...
		return animation.progress(animation.opts.progress).done(
	...
	function defaultPrefilter(elem, props, opts) {
		...
		var prop, value, toggle, tween, hooks, oldfire, anim = this, orig = {}, style = elem.style, hidden = elem.nodeType
				&& isHidden(elem), dataShow = data_priv.get(elem, "fxshow");
		...
		for (prop in props) {
			...
			if (rfxtypes.exec(value)) {
				...
				if (value === (hidden ? "hide" : "show")) {
					...
					// If there is dataShow left over from a stopped hide or
					// show and we are going to proceed with show, we should
					// pretend to be hidden
		...
		run : function(percent) {
			...
			if (this.options.duration) {
				this.pos = eased = jQuery.easing[this.easing](percent,
						this.options.duration * percent, 0, 1,
		...
		_default : {
			...
			set : function(tween) {
				...
				if (jQuery.fx.step[tween.prop]) {
					...
					jQuery
							.style(tween.elem, tween.prop, tween.now
									+ tween.unit);
				} else {
	...
	jQuery.each([ "toggle", "show", "hide" ], function(i, name) {
		...
		jQuery.fn[name] = function(speed, easing, callback) {
			return speed == null || typeof speed === "boolean" ? cssFn.apply(
					this, arguments) : this.animate(genFx(name, true), speed,
					easing, callback);
		};
	});
			...
			.extend({
				...
				animate : function(prop, speed, easing, callback) {
					...
					return empty || optall.queue === false ? this
							.each(doAnimation) : this.queue(optall.queue,
							doAnimation);
				},
				...
				finish : function(type) {
					...
					return this
							.each(function() {
								var index, data = data_priv.get(this), queue = data[type
								...
								// look for any active animations, and finish
								// them
								for (index = timers.length; index--;) {
									if (timers[index].elem === this
								...
								// look for any animations in the old queue and
								// finish them
								for (index = 0; index < length; index++) {
	...
	jQuery.fn.extend({
		...
		offsetParent : function() {
			return this.map(function() {
				...
				while (offsetParent
						&& (!jQuery.nodeName(offsetParent, "html") && jQuery
	...
	jQuery.each({
		...
		jQuery.fn[method] = function(val) {
			return jQuery.access(this, function(elem, method, val) {
				...
				if (win) {
					win.scrollTo(!top ? val : window.pageXOffset, top ? val
							: window.pageYOffset);
						...
						jQuery
								.each(
										...
										function(defaultExtra, funcName) {
											// margin is only for outerHeight,
											// outerWidth
